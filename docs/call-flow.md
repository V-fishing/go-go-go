...
flowchart TD
    ...
    L --> O[视觉观察采样 金框(仅日志/锚定)]
    O --> Q[弹窗应答 scan_popups]
...
1. **对方落子（确认盘面变化）**：连续 2/3 读确认有新增子 → `mover = 有 +1 的一方`，`turn = other(mover)`，同时 `TURN_LOCK['until'] = now + 15s`（锁窗防重复确认/重复点击，不再用于屏蔽视觉翻回合）。
...
`TURN_LOCK_SECS = 15.0`，`TURN_LOCK = {'until': 0.0}`，在对方落子确认成功后写 `TURN_LOCK['until'] = now + 15s`。锁窗期间屏蔽重复落子确认、防重复点击，并给落子动画稳定时间。

> 历史说明：旧版曾有「等待期视觉校正翻回合」逻辑——在锁窗外、连续两次金框判定为我方时，把回合翻回我方，以处理「对方虚着/停一手」的情形。该逻辑已删除：它会在开局/横幅滞后瞬间误判金框而翻错回合（如对方思考第一手时被强制翻回我方导致报错）。「对方虚着/停一手」现由弹窗处理器（`scan_popups` 命中 `pass`）正确接管，功能不丢失。
