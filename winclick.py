...
# 判据阈值: 占比 >= 0.10 即判定"我方行棋". 依据: 对方行棋时我方计时框为白框,
# ROI 内金框色占比理论 = 0, 故 0.10 门槛不会被对方行棋触发; 我方行棋时金框亮
# (完整罩住 ~0.24, 布局漂移被裁切时仍 ~0.168), 均 >= 0.10, 稳定判我方.
# 取 0.10 而非更低, 是给白框/木色底留余量, 避免纯噪声误触.
GOLD_THR = 0.03
# 金框精确色值 #F9FE7B (249,254,123). 判定 = ROI 内与 #F9FE7B 欧氏距离 < GOLD_TOL
# 的像素占比 >= GOLD_THR 即判"我方行棋".
# 注意(实测 TOL=32): 对方行棋时我方计时框金色描边常驻, ROI 内占比稳定 = 0.083,
# 已 >= 0.03 -> 会判为"我方行棋". 即阈值为 0.03 时, 对方回合也会触发我方行棋
# 判定(抢行棋风险). 该阈值仅在"对方行棋金框占比确为 0"的布局下安全.
GOLD_RGB = (249, 254, 123)
GOLD_TOL = 32                       # 与目标色 #F9FE7B 的欧氏距离容差


def gold_frame_ratio(rect=None, arr=None):
    """金色倒计时空心框占比: ROI 内"金框色 #F9FE7B"像素比例。

    主判据 = 与目标色 #F9FE7B(249,254,123) 的欧氏距离 < GOLD_TOL。
    实测(对局两态帧):
      我方行棋(我方计时框亮黄边) -> ROI 内占比 ~0.24, 最近像素精确=(249,254,123)
      对方行棋(我方计时框白底)   -> ROI 内占比 = 0.0000(完全无该色)
    注意: TOL 必须足够窄(见下方常量), 否则对方"浅黄白框"边框会被误算进金框像素,
    导致对方行棋占比虚高(~0.168)误判我方. 现 TOL=32 已排除该误判。
    出现该色块 = 我方行棋; 不出现 = 对方行棋。
    ...
    try:
        ...
        if x1 <= x0 or y1 <= y0:
            return None
        s = arr[y0:y1, x0:x1]
        R, G, B = (s[:, :, 0].astype(np.int32), s[:, :, 1].astype(np.int32),
                   s[:, :, 2].astype(np.int32))
        tr, tg, tb = GOLD_RGB
        d2 = (R - tr) ** 2 + (G - tg) ** 2 + (B - tb) ** 2
        return float((d2 < GOLD_TOL * GOLD_TOL).mean())
    except Exception:
        return None
